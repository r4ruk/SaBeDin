...
async fn process_body(user: FilteredUser, whole_body: &Bytes) -> Result<Vec<u8>, ApiError> {
    let mut json: Value = serde_json::from_slice(&whole_body).map_err(|_| ApiError::new(ErrorCode::BadRequest))?;
